    ...
    <div class="h-full flex flex-col">
        ...
        <div class="space-y-8 flex-grow overflow-y-auto">
            ...
            <div v-for="bag in bags" :key="bag.id" class="flex flex-col">
                ...
                <div class="flex items-center gap-4 bg-surface-700 p-3 rounded-lg shadow-md border-b-2 border-primary-500/50 mb-4">
                    
                    <div class="w-14 h-14 bg-surface-600 rounded-md flex items-center justify-center border border-primary-400 flex-shrink-0">
                    ...
                    <div class="flex-grow">
                        <h3 class="text-xl font-bold text-surface-0">
                            {{ bag.name }} 
                            <span class="text-surface-400 text-sm font-normal">({{ bag.items.filter(item => item !== null).length }} / {{ bag.slots }} slots)</span>
                        </h3>
                        <p class="text-surface-400 text-sm italic">Status: Equipped (Upgradeable)</p>
                    </div>
                </div>

                <div class="grid gap-2 p-2 rounded-lg bg-surface-700/30" :style="{ 'grid-template-columns': 'repeat(auto-fill, minmax(60px, 1fr))' }">
                    
                    <InventoryItemSlot 
                        v-for="(item, index) in bag.items" 
                        :key="index"
                        :item="item"
                        @click="(e) => (console.log('Open Inspector Modal for:', item?.name), openInspector(item))" 
                    />
                </div>
            </div>
        </div>

        <!-- RENDER THE MODAL AT THE BOTTOM OF THE INVENTORY VIEW -->
        <ItemInspectorModal />
    </div>
</template>