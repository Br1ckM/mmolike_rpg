...
<template>
    <div>
        ...
        <div class="grid grid-cols-5 gap-2">
            <div v-for="(item, index) in belt" :key="index" @click="useItem(index)"
                 class="relative"
            >
                <InventoryItemSlot :item="item" />
                
                <span class="absolute top-0 left-0 bg-surface-900/50 text-surface-300 text-[10px] px-1 rounded-br-md z-20">
                    F{{ index + 1 }}
                </span>
            </div>
        </div>
        <p class="text-surface-500 text-xs italic mt-3">Click a consumable to use it (simulated).</p>
    </div>
</template>