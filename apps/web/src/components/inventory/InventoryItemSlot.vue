// src/components/inventory/InventoryItemSlot.vue (Modified for Hover Text)
...
<template>
    ...
    >
        ...
        <span v-if="props.item && props.item.stackSize > 1" 
        ...
        <div v-if="props.item"
            class="absolute left-1/2 bottom-full mb-2 hidden group-hover:block w-max max-w-xs z-20 transform -translate-x-1/2
        ...
        >
            ...
            <p v-if="props.item.type !== 'gear'" class="text-surface-300 mt-1">Stack: {{ props.item.stackSize }}/{{ props.item.maxStack }}</p>
